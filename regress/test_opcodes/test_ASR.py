...
###############################################################################
#
# $Id: test_ASR.py,v 1.1 2004/07/31 00:59:11 rivetwa Exp $
#
...
class ASR_TestFail(base_test.TestFail): pass

class base_ASR(base_test.opcode_test):
	"""Generic test case for testing ASR opcode.

	ASR - Arithmetic ShiftRight
	opcode is '1001 010d dddd 0101' where d is register (0-31)
	...
	def setup(self):
		...
		return 0x9405 | (self.Rd << 4)

	def analyze_results(self):
		self.reg_changed.extend( [self.Rd, Reg.SREG] )
		
		# check that result is correct
		expect = ( ((self.Vd >> 1) & 0x7f) | (self.Vd & 0x80))

		got = self.anal_regs[self.Rd]
		
		if expect != got:
			self.fail('ASR r%02d: 0x%02x = (expect=%02x, got=%02x)' % (
		...
		if expect_sreg != got_sreg:
			self.fail('ASR r%02d: 0x%02x -> SREG (expect=%02x, got=%02x)' % (
...
#
template = """
class ASR_r%02d_v%02x_TestFail(ASR_TestFail): pass

class test_ASR_r%02d_v%02x(base_ASR):
	Rd = %d
	Vd = 0x%x
	def fail(self,s):
...
#
# automagically generate the test_ASR_rNN_vXX_rrNN_kXX class definitions.
#
code = ''
for d in range(32):
	for vd in vals:
