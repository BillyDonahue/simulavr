...
###############################################################################
#
# $Id: test_SBRS.py,v 1.1 2004/07/31 00:59:11 rivetwa Exp $
#

"""Test the SRBS opcode.
"""

import base_test
from registers import Reg

class SRBS_TestFail(base_test.TestFail): pass

class base_SRBS(base_test.opcode_test):
	"""Generic test case for testing SRBS opcode.

	The derived class must provide the reg member and the fail method.

	SRBS - Skip if Bit in Register is Set
	...
	def setup(self):
		...
		return 0xFE00 | (self.Rd << 4) | (self.b & 0x7)
	...
	def analyze_results(self):
		...
		if expect != got:
			self.fail('SRBS failed: expect=%x, got=%x' % (expect, got))
...
#
template = """
class SRBS_r%02d_b%d_v%02x_ni%d_TestFail(SRBS_TestFail): pass

class test_SRBS_r%02d_b%d_v%02x_ni%d(base_SRBS):
	Rd = %d
	b = %d
	v = %d
	ni = %d
...
#
# automagically generate the test_SRBS_* class definitions
#
code = ''
for d in range(32):
	for b in range(8):
